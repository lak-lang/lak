...
impl<'ctx> Codegen<'ctx> {
    ...
    pub fn write_object_file(&self, path: &Path) -> Result<(), CodegenError> {
        Target::initialize_native(&InitializationConfig::default()).map_err(|e| {
            CodegenError::without_span(format!("Failed to initialize native target: {}", e))
        })?;
        ...
        let cpu_str = cpu
            .to_str()
            .map_err(|_| CodegenError::without_span("CPU name contains invalid UTF-8"))?;
        let features_str = features
            .to_str()
            .map_err(|_| CodegenError::without_span("CPU features contain invalid UTF-8"))?;
